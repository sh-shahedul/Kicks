...
//             <Link to='/' className="flex items-center  text-sm font-semibold text-gray-900 whitespace-nowrap hover:opacity-75 transition-opacity">
//               New Drops <span>🔥</span>
//             </Link>
...
// pt-2 দেওয়া হয়েছে যাতে button আর dropdown এর মাঝে gap না থাকে
const DropdownMenu = ({ items }) => (
  ...
  return (
    <div className="flex flex-col items-center pt-8 md:px-12 px-5 max-w-7xl mx-auto">
      <nav className="w-full bg-white rounded-2xl shadow-sm px-6 py-6 relative">
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-4 pt-4 border-t border-gray-200 flex flex-col">
            <a href="#" className="flex items-center gap-1 px-2 py-3 text-sm font-semibold text-gray-900 hover:bg-gray-50 rounded-xl transition-colors">
              New Drops <span>🔥</span>
            </a>
            ...
            <div>
              <button
                className="w-full flex items-center justify-between px-2 py-3 text-sm font-semibold text-gray-900 hover:bg-gray-50 rounded-xl transition-colors"
                onClick={() => setMobileMenOpen(!mobileMenOpen)}
              >
                Men
                <span className={`transition-transform duration-200 ${mobileMenOpen ? "rotate-180" : ""}`}>
              ...
              {mobileMenOpen && (
                <div className="pl-4 flex flex-col mb-1">
                  {menItems.map((item) => (
                    <a key={item} href="#" className="px-2 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-colors">
                      {item}
                    </a>
                  ))}
                </div>
            ...
            {/* Search */}
            <a href="#" className="flex items-center gap-2 px-2 py-3 text-sm font-semibold text-gray-900 hover:bg-gray-50 rounded-xl transition-colors">
              <SearchIcon /> Search
            </a>
          </div>
        )}
